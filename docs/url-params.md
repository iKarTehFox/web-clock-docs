...
| clearLS | boolean | Clears all localStorage keys |
| clockMode | integer | Sets the clock display mode (`12`, `24`) |
| debugMode | boolean | Enables debug logging and Debugging menu options |
| language | string | Sets the site language (`en`, `es`) |
...
| mouseAutoHide | boolean | Enables/disables mouse auto-hide after inactivity |
| noUpdateNoti | boolean | Disables new update notifications |
...
| toastPosition | string | Sets the toast anchor position (`topleft`, `topmiddle`, `bottomleft`, `bottommiddle`, `bottomright`) |
...
**Example:** `?menuTheme=dark&panelVis=false&tabTitle=false&preset=owc-amoled-preset` loads the `owc-amoled-preset` preset file, enables the dark theme, hides the panel buttons, and disables the tab title.
...
**Example:** `?weatherApi=API_KEY&weatherLat=40.7128&weatherLon=-74.0060&weatherUnits=imperial` would load the weather widget with the specified API key, using imperial units, set to the coordinates of New York City.
